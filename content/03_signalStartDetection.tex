...
Results \todo{ist ja eigentlich noch keine Results section hier... Vielleicht "preliminary results / tests". Du kannst hier aber nicht einfach schon results mit reinbringen} show, that the accuracy of the whistle detection is not
...
For the entropy information, the signal must not be cleaned previously \todo{ist hier "must" im Sinne von "darf nicht" gemeint? "Muss nicht" ist zwar im Praesens möglich aber etwas unklar. Dann eher "does not have to be cleaned"}.
By looking at the derivation of the entropy, a global minimum can be found
at the starting point of the signal due to its change from noise to signal. \todo{in the figure, I don't really see a "global minimum". Is this because of the signal or does the entropy slowly rise after the start? The start index is clearly not at any "minimum". Maybe more explanation is needed here. On the other hand, one can clearly see that the entropy gives a good measure and clear results so that should not be a problem}
The starting index is therefore defined by the index of the derivation \todo{derivative's}'s minimum.
Of course, this approach does not work real-time \todo{is not real-time capable but introduces a small delay} but with small delay.
...
and its derivation is presented for evaluation. \todo{diesen Abschnitt lieber auch noch vor die figure oder komplett mit Absatz trennen}
...
Especially at the tournament \todo{rather "especially for competitive tournament conditions with varying and unforseeable noise levels, parameters...}, parameters like these should be avoided and thus,
...
contrary, signal noise is present at the end \todo{der Satz ergibt für mich so noch keinen Sinn. Meinst du "signal oder signal noise"? Vielleicht etwas mehr beschreiben oder den Satz umstellen}.
...
value to the first. \todo{das verstehe ich nicht aus diesem Satz.}
...
\begin{figure}[ht]
	...
	\label{fig:03_zcr}
\end{figure}
According to \todo{rather"Depending on" but that is personal preference...} the circumstances, the threshold can be lowered optionally.
