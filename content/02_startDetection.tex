...
\missing[inline]{
    Explain, why not only frequency is considered:\\
    \tab - Check, if other methods and combinations leads to higher accuracy.\\
    \tab - Light computing with ZCR\\
    \tab - Make it possible for signals that are not whistle
}
...
\bal
    P(n) &= \frac{E_s(n)}{\sum_{i=1}^N E(n_i)}
    \label{eq:02_pdf}\\
\eal
