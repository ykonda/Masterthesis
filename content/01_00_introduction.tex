...
different sensors \todo[inline]{Chen2006, cite, cite}.
\todo[inline]{Maybe enumerate two examples explicitly with "environment",
...
contact with an interacting human.\todo{Why is acoustic localization important for eye-contact?}
...
\todo{maybe one more citation here. Are all of them described in Brandstein96apractical}
\todo[inline]{You could also point to more modern methods if you have
literature. RNNs, CVAE. You could easily justify why these methods are not
considered in this work (due to computational effort)}

In this thesis, the \ac{TDOA} method was chosen to estimate the
\ac{SSL}\todo{"estimate the localization" seems incorrect. I think it would be
"sound source location" here. (OUTDATED:) Also, maybe write it out? Don't use
SS-location. Also, you might want to briefly justify the choice of TDOA as you
mentioned the alternatives earlier}. The performance of different such
strategies is evaluated with a focus on whistle sound localization in the
context of the \textit{\ac{RoboCup}}. \todo{Why textit (kursiv)? Write RoboCUP
SPL. Also, should you write "focus on" or simply say "in the context of" as you
don't really discuss things other than whistles?}
...
most appropriate solution.\todo{Oh I see. Here is the justification for why you
used TDOA. Maybe this up then.}
\todo[inline]{I think I would just end the introduction here and remove the
last sentence. IMHO too much detail for an introduction.}
...
It provides a platform that focuses on fast\todo{fast? LOL! Or fast as in
"almost"?} intelligent systems and multi-agent collaboration \cite{robocup}.
...
In the \ac{RoboCup} \ac{SPL}, software for humanoid robots is developed to
...
and many more \cite{nao_docu}.\todo{I don't think you have to enumerate all of them
here.}
% -------------------------------------------------------------
\begin{figure}[ht]
	\centering
...
2050. According to the current rules of the league\todo{cite SPL rule book},
...
limited to whistle sounds.\todo{is it?}
...
in \cref{subsec:03_microphones}.\todo{Maybe this should be part of the
introduction, not objective?}
...
\todo{Cite robocup statistics. Use "accuracy" and "precision" correctly rather
than hit rate} In practice, false positives only occur when whistles are are blown
