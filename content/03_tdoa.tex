...
two signals over each other and recording the similarity for each shift \todo{similarity muss definiert sein, Hier aber ja einfach: area under the curve of addition. }.
Thus, a peak will arise at that shift where signals are most similar.
Imaging\todo{meinst do "imaging" oder "imagining". Beides klingt aber irgendwie holprig. Nochmal drüber gucken!} two equal signals, one can image a peak at the middle of the \ac{CC} function
\lstinline!R!. \todo{maybe mentin that this is also the auto-correlation}
The index in this case is called \lstinline!zeroIndex! and calculated with
\lstinline!int(length(R))-1!.
If one signal is alike the other but delayed by some\todo{a number of} samples, the peak will
occur at so many samples next to the \lstinline!zeroIndex!. \todo{ich würde hier vielleicht mit einer Variablen arbeiten: " delayed by $n$ samples, the peak will occur at a shift of $n$ samples...}
...
in frequency domain. \todo{due to a performance advantage?}
...
The frame size in this work is set to 256 samples typically \todo{sind sie jetzt so gesetzt oder nicht? Am besten du kannst "typically" rausnehmen. Wenn du das doch variierst dann schreib das hier. So weiss der Leser nicht ob das jetzt immer so ist, manchmal oder nie} and the samples are Hann-windowed
prior to the correlation.
By zero padding the \ac{FFT}, resolution can be increased, but is refrained from for now. \todo{maybe put "due to higher computational effor" so you have some justification that should go well with the NAO system}
For two real signals, the \ac{CC} can be realized by time-reversing one of the signals before
the \ac{FFT} and then multiplying each component \todo{... element-wise in frequency domain. Würde ich hier nochal ausdruecklich schreiben, da es ja dadurch so effizient ist. and maybe be clearer about the steps: time-reversion, fft, element wise multiplication}.
In the event \todo{ist das ein event, dass manchmal auftritt oder eine bewusste Einstellung (dann eher "case")} of \ac{GCC-PHAT}, each component of the multiplication is divided by the absolute
...
is added to the delay estimation for both \ac{CC} and \ac{GCC}. \todo{give a few more implementation details. There probably are some parameters governing the resolution or fitting polynom or something..}
...
reference frequency value is limited to 2775.08\si{\hertz} as stated in the table above \todo{cref(table) nochmal}.
Evaluation of \cref{fig:04_diffFc} has shown, that the quality of the phase difference
method relies on the reference frequency to some extent. \todo{I don't think it is a good idea to reference into the results section and possibly the discussion. Rather say "preliminary test results" and then talk a little bit about it in the discussion. }
...
With this \ac{FFT} size, it \todo{$f_c$ statt it} can be selected between 2670.1\si{\hertz}
and 2756.25\si{\hertz}.
Frames after the signal start index are chosen, where a whistle is detected
in all channels by the same algorithm\todo{ich würde "as the existant w.d. " weglassen} as the existent whistle detection.
% -------------------------------------------------------------

\subsubsection*{Dynamic Reference Frequency}

In the other case \todo{other sensor pair oder was? Klarrer!}, the frame is chosen by doing a frequency analysis on
...
For better understanding, the $f_m$s \todo{das "s" sieht komisch aus... } per frame are plotted in \cref{fig:03_maxFreq}.
Here, one sees that the conditions apply at the beginning of the whistle signal. \todo{around frame number XXX -> to avoid confusion}
